...
| Check | Result |
|---|---|
| Test suite | 39 tests, all passing (expr 6, engine 7, import 6, assets 9, stage 11) |
| `tsc --noEmit` | Clean |
| `next build` | Clean |
...
- **MinerU never benchmarked.** It downloads models from HuggingFace or
  ModelScope on first run and both hosts are blocked from the build environment.
  Its commands in `tools/parse/README.md` are from documentation, not from a run.
...
- **Single-tenant** — no auth, no multi-user separation.

### Decision left open

The PR description was generated by the Claude Code UI, is **truncated
mid-sentence**, and predates the figure-support and parser-staging commits, so it
does not describe them. Worth rewriting before review.

---

## Suggested next step

Import one real past paper end to end — parse it, stage the figures, structure it,
and load it. That single pass will exercise the import aliases, the asset
validation, and the syllabus tagging against data none of this was written
against, and it is the cheapest way to find out which of the assumptions above
do not survive contact with a real Ten-Year-Series export.
